...
static void listbox_callback(GtkListBox *box, GtkListBoxRow *row, gpointer user_data) {
	...
	// Read TIM header from archive

	fseek(fp, ofs + 0x14, SEEK_SET);
	fread(&pallet_colors, 4, 1, fp);
	fread(&nb_pallets, 4, 1, fp);
	...
	*slash++;
	...
	switch(pallet_colors) {
		...
		break;
		default:
			fprintf(stderr, "Unknown pallet colors parameter\n");
			return;
		break;
	}
	...
	clut = (uint16_t*)malloc(pallet_colors * nb_pallets * sizeof(uint16_t));
	...
	switch(bpp) {
		case 4:
			...
			image_body = (uint16_t*)malloc(sizeof(uint16_t) * width * height);
			...
			printf("Converting body\n");

			convert_rgba5551(image_body, bitmap, width, height);
			free(image_body);
	...
	printf("Exporting tim\n");
	export_tim(bitmap, width, height);
	free(bitmap);
