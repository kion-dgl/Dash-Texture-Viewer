...
int main(int argc, char *argv[]) {
	...
	h_outline = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10);

	// Pack Vertical Outline
	
	label = gtk_label_new("Copyright 2017 Dashgl.com");
	gtk_widget_set_margin_bottom(GTK_WIDGET(label), 4);
	gtk_widget_set_margin_top(GTK_WIDGET(h_outline), 10);
	gtk_widget_set_margin_bottom(GTK_WIDGET(h_outline), 10);
	gtk_widget_set_margin_start(GTK_WIDGET(h_outline), 10);
	gtk_widget_set_margin_end(GTK_WIDGET(h_outline), 10);

	gtk_box_pack_start(GTK_BOX(v_outline), h_outline, TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(v_outline), label, FALSE, FALSE, 0);

	gtk_container_add(GTK_CONTAINER(window), v_outline);
